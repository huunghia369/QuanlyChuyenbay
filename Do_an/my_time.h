...
bool days_of_mon(date_time dt) {// check ngay cua thang
	if (dt.mon == 2) {
...
bool check_depart_time(date_time dt) { // kiem tra thoi gian khoi hanh
	...
	time_t now = time(0);          // trả về số giây kể tử 0:0 1/1/1970
	tm* ltm = localtime(&now);        // tgian hiện tại
	date_time dt_now;
	dt_now.day = ltm->tm_mday;				// ngày hiện tại
	dt_now.mon = 1 + ltm->tm_mon;			// trả về tháng hiện tại
	dt_now.year = 1900 + ltm->tm_year;   // trả về số năm tính từ 1900
